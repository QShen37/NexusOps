...
# 文件上传后的存储地址
UploadPath = config.upload_data_path

# 支持的文件类型
ALLOWED_EXTENSIONS = ["txt", "md"]

# 单个文件支持的最大大小
MAX_FILE_LENGTH = 16 * 1024 * 1024 # 10MB
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        if len(content) > MAX_FILE_LENGTH:
            raise HTTPException(status_code=400, detail=f"文件大小超过限制（最大 {MAX_FILE_LENGTH} 字节）")
        ...
        raise HTTPException(status_code=400, detail=f"文件上传失败：{e}")
